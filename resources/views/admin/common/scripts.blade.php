		...
		$.ajax({
			...
			success: function (res) {
				...
				if(res.length != ''){
					...
					for (i = 0; i < res.length; ++i) {
						var j = i + 1;
						showData[i] = "<tr><td>"+j+"</td><td>"+res[i].products_options_name+"</td><td>"+res[i].products_options_values_name+"</td><td>"+res[i].price_prefix+"</td><td>"+res[i].options_values_price+"</td><td>"+res[i].options_values_price_temp+"</td><td>    <a class='badge bg-light-blue editproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"'  language_id = '"+res[i].language_id+"'  options_id = '"+res[i].options_id+"'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> <a class='badge bg-red deleteproductattributemodal' products_attributes_id = '"+res[i].products_attributes_id+"' products_id = '"+res[i].products_id+"' language_id = '"+res[i].language_id+"'  options_id = '"+res[i].options_id+"'><i class='fa fa-trash' aria-hidden='true'></i></a></td></tr>";
...
{{--{{ URL::to('admin/deleteproductattributemodal')}}--}}
    ...
    //deletestoreModal
    $(document).on('click', '#storeFrom', function(){
  ...
  $(document).on('click', '#deleteModelId', function(){
    ...
    //getSource
    $(document).on('change', '#source_add_sale', function(){
        var selected_source = $(this).val();
        const urlParams = new URLSearchParams(window.location.search);
        urlParams.set('source', selected_source);
...
// ==================================================== PROMOTIONS =========================================================
$(document).on('change','.promo_product_from', function(){
    var product_id = $(this).val();
	$.ajax({
		url: '{{ URL::to("admin/products/inventory/get_promo_products_from")}}'+'/'+product_id,
		type: "GET",
		success: function (res) {
			//console.log(res);
			$('#attribute_from').html(res);
			$('#attribute_from').show();
...
$(document).on('change','.promo_product_to', function(){
    var product_id = $(this).val();
	$.ajax({
		url: '{{ URL::to("admin/products/inventory/get_promo_products_to")}}'+'/'+product_id,
		type: "GET",
		success: function (res) {
			//console.log(res);
			$('#attribute_to').html(res);
			$('#attribute_to').show();
			var has_val = $('#has-attribute').val();
			if(has_val==0){
				$('#attribute-btn').hide();
...
$(document).on('change','.product-type-bulk', function(){
    var product_id = $(this).val();

	$.ajax({
		url: '{{ URL::to("admin/products/inventory/ajax_min_max_bulk")}}'+'/'+product_id,
		type: "GET",
		success: function (res) {
			console.log(res.html);
			$('#attribute').html(res.html);
			$('#attribute').show();
...
jQuery(document).ready(function () {
    jQuery(document).on('change keyup', '#price_prefix, #options_values_price', function (e) {
        let selected_parent = jQuery(this).parentsUntil('#addAttributeModal');
        let final_price     = parseFloat(selected_parent.find('#final_price_default').val());
        let price_prefix    = selected_parent.find('#price_prefix').val();
        let adjusted_price  = parseFloat(selected_parent.find('#options_values_price').val());
        console.log(selected_parent);
        if(price_prefix == '+'){
            selected_parent.find('#final_price').val(final_price + adjusted_price);
