            ...
            <div class="col-12 col-md-7">
                ...
                <table class="table order-id">
                    <tbody>
                        ...
                        @if(isset($result['orders'][0]->pickup_store_id) and !empty($result['orders'][0]->pickup_store_id))
                        <tr class="d-flex">
                            <td class="col-6">@lang('labels.store')</td>
                            <td class="underline col-6">
                              @php
                                $pickup_id = $result['orders'][0]->pickup_store_id;
                                $store = DB::table('stores')->where('stores_id','=', $pickup_id)->first();
                                if($store) {
                                  echo $store->store_name .' | '.$store->store_email.' | '.$store->store_phone;
                                }
                              @endphp
                            </td>
                        </tr>  
              ...
              <div class="table-responsive ">
                  <table class="table order-table">
                      <tr>
                          ...
                          <td>{{Session::get('symbol_left')}} {{$price ?? 0}}</td>
                      </tr>
                      <tr>
                          <th>{{ trans('labels.Tax') }}:</th>
                          <td>{{Session::get('symbol_left')}} {{ $result['orders'][0]->total_tax }}</td>
                      </tr>
                      <tr>
                          <th>{{ trans('labels.ShippingCost') }}:</th>
                          <td>{{Session::get('symbol_left')}} {{ $result['orders'][0]->shipping_cost }}</td>
                      </tr>
                      @if(!empty($result['orders'][0]->coupon_code))
                          <tr>
                              <th>{{ trans('labels.DicountCoupon') }}:</th>
                              <td>{{Session::get('symbol_left')}} {{ $result['orders'][0]->coupon_amount }}</td>
                          </tr>
                      @endif
                      <tr>
                          <th>{{ trans('labels.Total') }}:</th>
                          <td>{{Session::get('symbol_left')}} {{ $result['orders'][0]->order_price }}</td>
                      </tr>
                  </table>
              </div>
          </div>
            ...
            <div class="col-xs-12 col-sm-12">
                @if(count($result['orders'][0]->statusess)>0)
                    <div style="border-radius:5px;"class="card">
                        ...
                        <div class="card-body">
                        @if(empty($result['orders'][0]->statusess))
