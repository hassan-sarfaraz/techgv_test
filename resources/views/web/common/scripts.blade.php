<script src="{{asset('web/js/typeahead.bundle.min.js')}}"></script>
<script>
    var ajaxsearch = new Bloodhound({
        datumTokenizer: Bloodhound.tokenizers.obj.whitespace('value'),
        queryTokenizer: Bloodhound.tokenizers.whitespace,
        remote: {
            url: '{{ URL::to("/ajaxsearch/") }}?query=%QUERY',
            wildcard: '%QUERY'
        }
    });

    jQuery('.search input').typeahead(null, {
        highlight: false,
        hint: false,
        limit: 5,
        minLength: 3,
        ...
        templates: {
            ...
            suggestion: function(data) {
                return '<div class="ajaxsearch_row"><div><img style="height:50px;width:50px" src='+data.image+'></div><div><a href="'+data.link+'">' + data.name + '</a></div></div>';
            }
        }
    });
</script>
...
<script type="application/javascript">
    ...
    jQuery( document ).ready( function () {
        if (!sessionStorage.getItem('shown-home-promo')){
            //jQuery('#promotionPop').modal({show:true});
                ...
                jQuery.ajax({
                    ...
                    success: function (res) {
                        ...
                        if(res.status == 'exceed'){
                            ...
                            swal("Congrats!", "Product Added Successfully Thanks.Continue Shopping", "success");
                            @else
...
//sortby
        ...
        jQuery( function() {
            jQuery.widget( "custom.iconselectmenu", jQuery.ui.selectmenu, {
                _renderItem: function( ul, item ) {
                    var li = jQuery( "<li>" ), wrapper = jQuery( "<div>", { text: item.label } );
                    if ( item.disabled ) {
            ...
            jQuery.ajax({
                ...
                success: function (res) {
                    ...
                    if(res['status'] == 'exceed'){
                        ...
                        jQuery('.head-cart-content').html(res);
                        jQuery(parent).addClass('active');
                        // swal("Congrates!", "<a href='' class='btn bg-light btn-lg btn-block'>Continue shopping</a><br><a href='' class='btn btn-success btn-lg btn-block'>View cart</a>", "success",{button: false});
                        swal({
                            html: true,
                            title: "<i>Product was added to the bag</i>",
                            text: "<a href='{{url('shop')}}' class='btn bg-light btn-lg btn-block'>Continue shopping</a><br><a href='{{url('viewcart')}}' class='btn btn-success btn-lg btn-block'>View cart</a>",
                            type:"success",
        ...
        jQuery(document).on('click', '.qtyplus', function(e){
            ...
            @if(!empty($result['promotions']))
                ...
                @if(!empty($promotion))
            if( attributes_value == {{ !empty($promotion) ? $promotion->promotion_attribute_id : 0 }}
                && parseFloat(qty) >= parseFloat({{$promotion->buy_frequency}}) ){
                console.log("qty", qty)
            ...
            if(parseFloat({{$promotion->buy_frequency}}) > parseFloat({{$promotion->get_frequency}})){
                jQuery(".qty_promo-{{ $promotion->promotion_attribute_id }}").val(parseInt(parseFloat(qty) / parseFloat({{$promotion->buy_frequency}})))
            }else{
                jQuery(".qty_promo-{{ $promotion->promotion_attribute_id }}").val(parseInt(parseFloat(qty) * parseFloat({{$promotion->get_frequency}})))
            }
            @endif;
            @endforeach
                @endif;
    ...
    function paymentMethods(){
        ...
        var pickup_store_id = jQuery("#pickup_store").val();
        ...
        jQuery.ajax({
            ...
            type: "POST",
            data: '&payment_method='+payment_method+'&pickup_store_id='+pickup_store_id,
            success: function (res) {
                //jQuery('#loader').hide();
            },
        });
    ...
    //pay_payd
    jQuery(document).on('click', '#pay_payd', function(e){
        var formData = jQuery("#payd_form").serialize();
        jQuery.ajax({
            headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
            url: '{{ URL::to("/pay-payd")}}',
            type: "POST",
    ...
    jQuery(document).ready(function() {
        ...
        jQuery(document).on('change', '.{{ $functionValue }}', function(e){
            ...
            @if(!empty($result['promotions']))
                @foreach($result['promotions'] as $promotion)
            if( attributes_value == {{ !empty($promotion) ? $promotion->promotion_attribute_id : 0 }}
                && parseFloat(qty) >= parseFloat({{!empty($promotion) ? $promotion->buy_frequency : 0}}) ){
                jQuery(".promotion_product-{{ $promotion->promotion_attribute_id }}").show()
            }else{
                jQuery(".promotion_product-{{ $promotion->promotion_attribute_id }}").hide()
            }
...
<script>
 ...
 <script>
        $(document).ready(function() {
            var contCode = jQuery('.iti__selected-flag').attr('title');
            var contCode = contCode.replace(/[^0-9]/g,'')
            jQuery('#phone').val("");
            jQuery('#phone').val("+"+contCode+" "+ jQuery('#phone').val());
