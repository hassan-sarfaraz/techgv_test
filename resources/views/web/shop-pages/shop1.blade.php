         ...
         <div class="col-12 col-lg-3  d-lg-block d-xl-block right-menu">
           <div class="right-menu-categories">
            @if(!empty($result['categories']))
             ...
             <a class=" main-manu"  @if(isset($category->childs)) href="#{{$category->slug}}" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="men-cloth" @else href="{{url('shop?category=').$category->slug}}" @endif>
               <img class="img-fuild" src="{{asset($category->image_path)}}">{{$category->categories_name}} <span><i class="fas fa-minus"></i></span>
             </a>
           ...
           <form enctype="multipart/form-data" name="filters" id="test" method="get">
             ...
             <input type="hidden" name="filters_applied" id="filters_applied" value="1">
              @if(!empty($result['filter_attribute'])){
             ...
             @include('web.common.scripts.slider')
                   ...
                   @if($attr_data['option']['name'] != 'Waist')
             ...
             <div class="breadcum-area">
               @if(!empty($result['category_name']) and !empty($result['sub_category_name']))
                 ...
                 @else
                  ...
                  @else
                  <!-- <h2>الساعات والاكسسوارات</h2> -->
                  <!-- <h5>النظارات الشمسية والساعات والملابس الرجالية وأكثر من ذلك بكثير...</h5> -->
                  @endif
                               ...
                               <article>
                                ...
                                if($after_date>=$current_date){
                                 ...
                                 <div class="price">
                                   ...
                                   <span> {{Session::get('symbol_left')}}{{pd_adjust_price_with_tax($orignal_price+0)}}{{Session::get('symbol_right')}}</span>
                                   @else
