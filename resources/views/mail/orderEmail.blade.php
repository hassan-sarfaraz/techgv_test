<div style="padding: 5px;max-width: 950px;margin: 0 auto;">

  <div style="text-align:center;background-color: #47c257;padding: 15px 0;">
    <div style="font-size:28px;font-weight:bold;margin-bottom:15px;text-transform:uppercase">
      CONGRATULATIONS!<br>YOUR ORDER HAS BEEN PLACED.
    </div>
    <div style="font-size:12px;line-height:18px;width:100%;margin-bottom:15px">
      Hi {{ $ordersData['orders_data'][0]->customers_name }},
    </div>
    <div style="font-size:12px;line-height:18px;width:100%">
      Thank you for placing an order with Under Protien District. We are happy to inform you that your order of the following item(s) has been received:
    </div>
  </div>
  ...
  <!-- info row -->
  <div style="display: flex;width: 100%;padding: 0 0 20px;flex-direction: row;flex-wrap: wrap;align-content: space-around;justify-content: space-between;align-items: flex-start;">
  ...
  <table class="table table-striped" style="width: 100%;background-color: transparent;margin: 15px 0 15px;">
    <thead>
      <tr>
        ...
        <th align="left">{{ trans('labels.ProductName') }}</th>
        <th align="left">{{ trans('labels.AdditionalAttributes') }}</th>
    ...
    <tbody style="text-transform: capitalize;font-size: 12px;">
     @foreach($ordersData['orders_data'][0]->data as $key=>$products)
       @php
        $p_image = \Illuminate\Support\Facades\DB::table('image_categories')
                  ->where('image_id','=', $products->image)
      ...
      <tr @if($key%2==0) style="background-color: #f9f9f9;" @endif>
        ...
        @if($key%2==0) <td align="center" style="border-top: 1px solid #f4f4f4; padding: 8px;"> @else <td align="center" style="padding: 8px;"> @endif<img src="{{ asset($p_image->path ?? "") }}" width="60px"> </td>
        @if($key%2==0) <td align="left" style="border-top: 1px solid #f4f4f4; padding: 8px;"> @else <td align="left" style="padding: 8px;"> @endif  {{  $products->products_name }}<br></td>
        @if($key%2==0) <td align="left" style="border-top: 1px solid #f4f4f4; padding: 8px;"> @else <td align="left" style="padding: 8px;"> @endif
