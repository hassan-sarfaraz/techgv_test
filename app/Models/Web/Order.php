...
class Order extends Model
{
	...
	public function place_order($request)
	{
		...
		$order_total_cal = session('products_price');
		//$tax_rate = number_format((float)session('tax_rate'), 2, '.', '');
		$tax_rate = pd_adjust_price_with_tax($order_total_cal) - ($order_total_cal);
		...
		if ($payment_method == 'braintree') {
			...
			$payment_status = 'success';
		} else if ($payment_method == 'pickup_from_store') {
			$payments_setting = $this->payments_setting_for_pfstore();

			$payment_method = $payments_setting->name;
			$payment_status = 'success';

			$pickup_from_store_id = session()->get('pickup_store_id', 0);
		...
		} else if ($payment_method == 'Payd') {
			$payd =  $this->payments_setting_for_payd();
			$payment_method = $payd['secret_key']->name;
			$payment_status = 'success';
			$order_information = $request->nonce;
		} else if ($payment_method == 'pay_with_credit_card') {
			$payd =  $this->payments_setting_for_postpay_full_installment();
			$payment_method = $payd['secret_key']->name;
			$payment_status = 'success';
			$order_information = $request->nonce;
		} else if ($payment_method == 'pay_in_installments') {
			$payd =  $this->payments_setting_for_postpay_installments();
			$payment_method = $payd['secret_key']->name;
			$payment_status = 'success';
			$order_information = $request->nonce;
		...
		if ($payment_status == 'success') {
			...
			$ret['status'] = $payment_status;
			$ret['orders_id'] = $orders_id;
			return $ret;
	...
	public function payments_setting_for_pfstore()
	{
		$payments_setting = DB::table('payment_description')
			->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_description.payment_methods_id')
			->select('payment_description.name', 'payment_methods.environment', 'payment_methods.status', 'payment_methods.payment_method')
	...
	public function payments_setting_for_payd()
	{
		$payments_setting = DB::table('payment_methods_detail')
			->leftjoin('payment_description', 'payment_description.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
			->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
	...
	public function payments_setting_for_postpay_full_installment()
	{
		$payments_setting = DB::table('payment_methods_detail')
			->leftjoin('payment_description', 'payment_description.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
			->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
	...
	public function payments_setting_for_postpay_installments()
	{
		$payments_setting = DB::table('payment_methods_detail')
			->leftjoin('payment_description', 'payment_description.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
			->leftjoin('payment_methods', 'payment_methods.payment_methods_id', '=', 'payment_methods_detail.payment_methods_id')
