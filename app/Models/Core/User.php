...
class User extends Authenticatable
{
    ...
    public static function getCustomers(){
      $user = User::sortable(['id'=>'ASC'])
          ...
          ->where('role_id',2)
          ->select('users.id', 'users.role_id', 'users.user_name', 'users.first_name', 'users.last_name', 'users.gender', 'users.default_address_id', 'users.country_code as user_country_code', 'users.phone', 'users.email', 'users.password', 'users.avatar', 'users.status', 'users.is_seen', 'users.phone_verified', 'users.remember_token', 'users.auth_id_tiwilo', 'users.dob', 'users.created_at', 'users.updated_at', 'address_book.entry_gender as entry_gender', 'address_book.entry_company as entry_company',
