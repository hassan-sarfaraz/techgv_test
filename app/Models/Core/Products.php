...
class Products extends Model
{
	...
	public function paginator($request){
        ...
        $data->select('products.*', 'products_description.language_id','products_description.products_name','products_description.products_description','products_description.products_url','products_description.products_viewed','products_description.products_left_banner','products_description.products_left_banner_start_date','products_description.products_left_banner_expire_date','products_description.products_right_banner','products_description.products_right_banner_start_date','products_description.products_right_banner_expire_date', 'specials.specials_id',
        'manufacturers.manufacturer_name', 'manufacturers.manufacturer_image', 'manufacturers.manufacturers_slug', 'manufacturers.date_added', 'manufacturers.last_modified', 'manufacturers.manufacturers_image',
        ...
        if (isset($_REQUEST['categories_id']) or isset($_REQUEST['product'])) {
  ...
  public function getter(){
            ...
            //  }
              ...
              $data->select('products.products_id as p_id', 'products.products_quantity', 'products.products_model', 'products.products_image', 'products.products_price', 'products.products_date_added', 'products.products_last_modified', 'products.products_date_available', 'products.products_weight', 'products.products_weight_unit', 'products.products_status', 'products.is_current', 'products.products_tax_class_id', 'products.manufacturers_id as p_manufacturers_id', 'products.products_ordered', 'products.products_liked', 'products.low_limit', 'products.is_feature', 'products.products_slug', 'products.products_type', 'products.products_min_order', 'products.products_max_stock', 'products_description.*', 'specials.specials_id', 'manufacturers.*', 'specials.products_id as special_products_id', 'specials.specials_new_products_price as specials_products_price', 'specials.specials_date_added as specials_date_added', 'specials.specials_last_modified as specials_last_modified', 'specials.expires_date', 'image_categories.path as path',
              ...
              if (isset($_REQUEST['categories_id']) and !empty($_REQUEST['categories_id'])) {
                  ...
                  $products = $data->orderBy('p_id', 'DESC')->paginate($commonsetting['pagination']);
  ...
  public function insert($request){
    ...
    foreach($languages as $languages_data){
        ...
        $products_details = 'products_details_'.$languages_data->languages_id;
        //left banner
        ...
        $req_products_details = $request->$products_details;
        DB::table('products_description')->insert([
            ...
            'products_details' => addslashes($req_products_details)
    ...
    foreach($result['languages'] as $languages_data){
      $description = DB::table('products_description')
        ...
        if(count($description)>0){
            ...
            $description_data[$languages_data->languages_id]['products_details'] = $description[0]->products_details;
            ...
            $description_data[$languages_data->languages_id]['products_details'] = '';
  ...
  public function get_promo_attributes_variables($id){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $language_id      =   '1';
    ...
    $result['min_level'] = $min_level;
    $result['max_level'] = $max_level;
    $products_attribute = DB::table('products_attributes')->where('products_id', '=', $products_id)->where("is_default", '=', 0)->get();
    $products_attribute = $products_attribute->unique('options_id')->keyBy('options_id');
    if(count($products_attribute)>0){
        $index2 = 0;
        foreach($products_attribute as $attribute_data){
  ...
  public function ajax_min_max_bulk($id){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $language_id      =   '1';
    ...
    $sizesArray = [];
    $flavorsArray = [];
    foreach($result['attributes'] as $newAttribute){
        if($newAttribute['option']['name'] == 'Size'){
            $sizesArray[] = $newAttribute['values'];
            // dd($newAttribute['values']);
        }
    }
    foreach($result['attributes'] as $newAttribute){
        if($newAttribute['option']['name'] == 'Flavours'){
            $flavorsArray[] = $newAttribute['values'];
        }
    }
    $result['sizesArray'] = $sizesArray[0];
    $result['flavorsArray'] = $flavorsArray[0];
    $sizeAttributeIdsArray = [];
    foreach($result['sizesArray'] as $sizeArray){
        $sizeAttributeIdsArray[] = $sizeArray['products_attributes_id'];
    }
    $sizeAttributeIds = implode(",",$sizeAttributeIdsArray);

    $html = '<table class="table table-bordered table-striped">';
    foreach($result['sizesArray'] as $sizeArray){
        
        // dd($sizeArray);
        $html .= '<tr style="background:lightgrey">';
        $html .= '<td colspan="3">'.$sizeArray['value'].'</td>';
        $html .= '</tr>';

        // dd($sizesArray[0]);
        // foreach($result['flavorsArray'] as $flavorArray){
            // dd($flavorArray);

            $attrId = $sizeArray['products_attributes_id'];

            $qry = DB::Select("SELECT 

            products_attributes.*,products_options.`products_options_name`,products_options_values.`products_options_values_name`,
            COALESCE((SELECT SUM(stock) FROM inventory WHERE inventory.`inventory_ref_id` IN (
            SELECT inventory_ref_id FROM inventory_detail WHERE inventory_detail.`inventory_ref_id` IN(
            SELECT inventory_detail.`inventory_ref_id` FROM inventory_detail WHERE inventory_detail.`products_id`=$products_id AND inventory_detail.`attribute_id` IN (products_attributes.`products_attributes_id`))
            ...
            SELECT inventory_detail.`inventory_ref_id` FROM inventory_detail WHERE inventory_detail.`products_id`=$products_id AND inventory_detail.`attribute_id` IN (products_attributes.`products_attributes_id`))
            AND attribute_id=$attrId)
            AND stock_type='out'),0) Stock 
            FROM products_attributes 
            INNER JOIN products_options ON products_options.`products_options_id`= products_attributes.`options_id`
            INNER JOIN products_options_values ON products_options_values.`products_options_values_id`=products_attributes.`options_values_id`
            WHERE products_id=$products_id AND products_attributes_id NOT IN ($sizeAttributeIds)");
            // $qryData = $qry;
            // dd($qry);
            foreach($qry as $qryAttribute){

            $html .= '<input type="hidden" name=sizeAttributes['.$attrId.'][] value='.$qryAttribute->products_attributes_id.' class="form-control">';

                // dd($qryAttribute);
                $html .= '<tr>';
                $html .= '<td>'.$qryAttribute->products_options_values_name.'</td>';
                $html .= '<td>'.$qryAttribute->Stock.'</td>';
                $html .= '<td>';
                $html .= '<input type="text" name=stocks['.$attrId.']['.$qryAttribute->products_attributes_id.'] class="form-control">';
                $html .= '</td>';
                $html .= '</tr>';
            }
  ...
  public function addnewstockBulk($request){
    $products_id = $request->products_id;
    $language_id     =   1;
    $product = DB::table('products')
                ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
            ...
            $products    = $product;
            $date_added	 = date('Y-m-d h:i:s');
            foreach($request->stocks as $index => $sizeAttribute){
                foreach($sizeAttribute as $flavorAttribute => $stock){
                    if(empty($stock)){
                        continue;
                    }
                    $stock_type  = "in";
                    ...
                    $inventory_ref_id = DB::table('inventory')->insertGetId([
                        'products_id'       => $products_id,
                        'reference_code'    => $request->reference_code ?: 0,
                        'stock'             => $stock_value,
  ...
  public function addinventoryfromsidebarBulk(){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $language_id      =   '1';
    ...
    ## Promotions Starting
    public function addnewpromotion($request){

        $products_id_from = $request->products_id_from;
        $products_id_to = $request->products_id_to;
        $offer_type_from = $request->offer_type_from;
        $offer_type_to = $request->offer_type_to;
        $language_id     =   1;
        $buy_frequency = $request->buy_frequency;
        $get_frequency = $request->get_frequency;

        $start = Carbon::createFromFormat('d/m/Y', $request->start)->format('Y-m-d');
        $end = Carbon::createFromFormat('d/m/Y', $request->end)->format('Y-m-d');//Carbon::parse($request->end)->format('Y-m-d');

        $offer_plateform = $request->offer_platform;
        $is_active = $request->is_active;

        ## check variable products
        $date_added	= date('Y-m-d h:i:s');
//         print_r($request->all());exit;
        // foreach($request->attribute_id_from as $attribute){
        //     print_r("from");
        //     print_r($attribute);
        // }
        // foreach($request->attribute_id_to as $attribute){
        //     print_r("to");
        //     print_r($attribute);
        // }
        // exit();

        $promotion_id = DB::table('promotions')->insertGetId([
            'product_id_from'=>$products_id_from,
            'product_id_to'=>$products_id_to,
            'start' => $start,
            'end' => $end,
            'buy_frequency' => $buy_frequency,
            'get_frequency' => $get_frequency,
            'offer_plateform'=> $offer_plateform,
            'is_active' => $is_active,
            'offer_type' => $offer_type_from,
            'created_at'=> $date_added,
            'updated_at'=> $date_added
        ]);
        // print_r($request->attributeid);
        // print_r("====");
        // print_r($products_id_from);
        // print_r($products_id_to);
        // exit();
        ## get check product type variables or not
        if( isset($promotion_id)){
            if($offer_type_from == 'variable_product'){
                if(isset($request->attributeid[$products_id_from])){
//                    foreach($request->attributeid[$products_id_from] as $attribute){

                        if(!empty( $attribute = $request->attributeid[$products_id_from][0] )){
                            DB::table('promotion_attributes')->updateOrInsert([
                                'promotion_id'=> $promotion_id,
                                'product_id'=>$products_id_from,
...
//                    }
            ...
            if($offer_type_to == 'variable_product'){
                if(isset($request->attributeid[$products_id_to])){
//                    foreach($request->attributeid[$products_id_to] as $attribute2){

                        if(!empty( $attribute2 = $request->attributeid[$products_id_from][1] )){
                            DB::table('promotion_attributes')->updateOrInsert([
                                'promotion_id'=>$promotion_id,
                                'product_id'=>$products_id_to,
                                'attribute_id' => $attribute2,
                                'created_at'=> $date_added,
                                'updated_at'=> $date_added,
                                'p_type'=>'to'
                            ]);
...
//                    }
    ...
    public function promotion_update($request){
        ...
        $date_added	= date('Y-m-d h:i:s');
        $promotions = DB::table('promotions')->where('promotions.id', '=', $promotion_id);

        $promo_from = $promotions->first()->product_id_from;
        $promo_to = $promotions->first()->product_id_to;

        if( $offer_type_from != '' && $offer_type_to != ''){
            // both new products
            ...
            // Removed and create new product ids
            $promotion_attributes = DB::table('promotion_attributes')->where('promotion_attributes.promotion_id', '=', $promotion_id);
            if(isset($promotion_attributes)){
                $promotion_attributes->delete();
            }
            if($offer_type_from == 'variable_product'){
                if(isset($request->attributeid[$products_id_from])){
                    foreach($request->attributeid[$products_id_from] as $attribute){
        ...
        // if from same product
        elseif( $offer_type_from != '' && $offer_type_to == '' ){
            $promotion_update = $promotions ->update([
                'product_id_from'=>$products_id_from,
                'start' => $start,
                'end' => $end,
            ...
            // product_from
            $promotion_attributes_from = DB::table('promotion_attributes')->where('promotion_attributes.promotion_id', '=', $promotion_id)->where('promotion_attributes.product_id', '=', $promo_from);
            if(isset($promotion_attributes_from)){
                $promotion_attributes_from->delete();
            }
            if($offer_type_from == 'variable_product'){
                if(isset($request->attributeid[$products_id_from])){
                    foreach($request->attributeid[$products_id_from] as $attribute){
        ...
        elseif( $offer_type_from == '' && $offer_type_to != '' ){
            $promotion_update = $promotions ->update([
                ...
                'offer_plateform'=> $offer_plateform,
                'is_active' => $is_active,
                'offer_type' => $offer_type_to,
                'updated_at'=> $updated_at
            ]);


            // product_to
            $promotion_attributes_to = DB::table('promotion_attributes')->where('promotion_attributes.promotion_id', '=', $promotion_id)->where('promotion_attributes.product_id', '=', $promo_to);
            if(isset($promotion_attributes_to)){
                $promotion_attributes_to->delete();
            }
            if($offer_type_to == 'variable_product'){
                if(isset($request->attributeid[$products_id_to])){
                    foreach($request->attributeid[$products_id_to] as $attribute){
                        if(!empty($attribute)){
                            DB::table('promotion_attributes')->updateOrInsert([
                                'promotion_id'=> $promotion_id,
                                'product_id'=>$products_id_to,
    ...
    public function promotion_list($request){
        $result = Promotion::orderBy('created_at', 'desc')->get();
        return $result;
    }
    public function promotion_delete($request){
        $result = DB::table('promotions')->where('promotions.id', $request->id)->delete();
        if($result){
            DB::table('promotion_attributes')->where('promotion_attributes.promotion_id', $request->id)->delete();
        }
        return $result;
    }
    public function promotion_edit($request){
        ...
        $promotions = DB::table('promotions')->where('promotions.id', $request->id)->get();
        $products_id = $promotions[0]->product_id_from;
        $result["promotion_attributes_ids"] = DB::table('promotion_attributes')->where('promotion_attributes.promotion_id', $request->id)->pluck('attribute_id')->toArray();
  ...
  public function addproductattribute($request){
    ...
    $products_price = DB::table('products')->select('products_price')->where('products_id','=', $products_id)->first()->products_price ?: 0;
    $result['options_value'] = $options_value;
    $result['data'] = array('products_id' => $products_id, 'products_price' => $products_price);
  ...
  public function updateoption($request){
    ...
    if(empty($request->price_prefix)) {
        $request->price_prefix = "-";
    }

    $product_data = DB::table('products')->select('products_id','products_price')->where('products_id','=', $request->products_id)->first();

    if($product_data) {
        $product_original_price = $product_data->products_price;
        if(empty($request->options_values_price)) {
            $request->price_prefix = '-';
            $request->options_values_price = 0;
        }else if($product_original_price > $request->options_values_price) {
            $request->price_prefix = '-';
            $request->options_values_price = ($product_original_price - $request->options_values_price);
        }else{
            $request->price_prefix = '+';
            $request->options_values_price = ($product_original_price + $request->options_values_price);
        }
    ...
    $products_attributes = DB::table('products_attributes')
        ...
        $products_attributes_adjusted = [];
        if(count($products_attributes) > 0) {
            foreach($products_attributes as $key => $products_attribute) {
                $products_attribute = (array) $products_attribute;
                $products_attributes_adjusted[] = $products_attribute;
                if($products_attribute['price_prefix'] == '-') {
                    $products_attributes_adjusted[$key]['options_values_price'] = (($product_original_price - $products_attribute['options_values_price']));
                    $products_attributes_adjusted[$key]['options_values_price_temp'] = pd_adjust_price_with_tax(($product_original_price - $products_attribute['options_values_price']));
                }else{
                    $products_attributes_adjusted[$key]['options_values_price'] = (($product_original_price + $products_attribute['options_values_price']));
                    $products_attributes_adjusted[$key]['options_values_price_temp'] = pd_adjust_price_with_tax(($product_original_price + $products_attribute['options_values_price']));
                }
        ...
        return collect($products_attributes_adjusted);
  }
