...
class ReportsController extends Controller
{
	...
	public function productSaleReport(Request $request){
		...
		if($reportBase=='this_month'){
			...
			for($j = 1; $j <= $dateLimit; $j++){
				...
				//sold products
				$orders = DB::table('orders')->whereBetween('date_purchased', [$dateFrom, $dateTo]);

                if(!empty($source)) {
                    $orders->where('ordered_source','=', $source);
                }
    ...
    public function conversionRate(Request $request) {
        $title = array('pageTitle' => 'Conversion Rate Report');
        $language_id = 1;
        return view("admin.reports.conversionRate",$title);
    }

    public function topSearchedkeywords(Request $request) {
        $title = array('pageTitle' => 'Top 50 Google Search Terms');
        $language_id = 1;
        return view("admin.reports.topSearchedKeywords",$title);
    }

    public function bestPerformingCategories(Request $request) {
        $title = array('pageTitle' => 'Best Performing Categories');
        $language_id = 1;
        return view("admin.reports.bestPerformingCategories",$title);
    }

    public function bestSellingProducts(Request $request) {
        $title = array('pageTitle' => 'Best Selling Products');
        $language_id = 1;
        $limit       = 20;
        $result = [];
        $result['limit'] = $limit;
        ...
        $orders_products = DB::table('orders_products')
                            ...
                            ->select('products.products_liked', 'orders_products.*', 'image_categories.path as image', DB::raw('SUM(orders_products.products_quantity) as total'))
                            ->groupBy('orders_products.products_id','orders_products.products_name')
                            ->orderBy('total','desc')
                            ->limit($limit)
                            ->get();
        ...
        return view("admin.reports.bestSellingProducts",$title)->with('result', $result);
    }
}
