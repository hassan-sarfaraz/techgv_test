	...
	public function dashboard(Request $request)
	{
		...
		foreach ($orders as $orders_data) {
			...
			if ($orders_data->orders_status_id == '2') {
				$compeleted_orders++;
				$total_money_earned = $total_money_earned + $orders_data->order_price;
			}
		...
		$result['total_money_earned'] = $total_money_earned;
		...
		foreach ($products as $products_data) {
			...
			if (count($currentStocks) > 0) {
				...
				if ($products_data->products_type != 1) {
					...
					if (($c_stock_in - $c_stock_out) <= $min_level) {
						$lowOfStock++;
					}
