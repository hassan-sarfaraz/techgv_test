...
class OrdersController extends Controller
{
    ...
    public function display(Request $request)
    {
        ...
        $orders         = DB::table('orders')->where('customers_id', '!=', '')->where('payment_status', '=', 1);
        if (!empty($request->customers_name)) {
            $orders = $orders->where('customers_name', $request->customers_name);
        }
        if (!empty($request->date_purchased)) {
            $create_from_date   = date_create($request->date_purchased);
            $date               = date_format($create_from_date, "Y-m-d");
            $orders             = $orders->whereDate('date_purchased', $date);
        }

        $orders         = $orders->orderBy('date_purchased', 'DESC')->paginate(40);
        $index          = 0;
        $total_price    = array();
        ...
        foreach ($orders as $orders_data) {
            ...
            $orders[$index]->orders_status_id = $orders_status_history[0]->orders_status_id ?? 0;
    ...
    public function unpaidOrder(Request $request)
    {
        ...
        $orders         = DB::table('orders')->where('customers_id', '!=', '')->where('payment_status', '=', 0);
        if (!empty($request->customers_name)) {
            $orders = $orders->where('customers_name', $request->customers_name);
        }
        if (!empty($request->date_purchased)) {
        ...
        $ordersData['orders'] = $orders;
        $ordersData['currency'] = $this->myVarsetting->getSetting();
        return view("admin.Orders.unpaidorder", $title)->with('listingOrders', $ordersData);
    }
    ...
    public function updateOrder(Request $request)
    {
        ...
        if ($old_orders_status == $orders_status) {
            ...
            ////////sms send on order stats change
            if ($data['status'] == "Completed") {
                $message = "Dear Waqar, Your order " . $data['orders_id'] . " has been shipped to your shipping address.";
            } else {
                $message = "Dear Waqar, We are working on your Order" . $data['orders_id'] . " It will be delivered within 48 hours.";
            }
