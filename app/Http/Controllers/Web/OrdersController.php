...
class OrdersController extends Controller
{
	...
	{
		// return $test=config('app.postpay');
		$title = array('pageTitle' => Lang::get('website.Checkout'));
	...
	public function paymentComponent(Request $request)
	{
		...
		session(['pickup_store_id' => 0]);
		if (isset($request->pickup_store_id) and !empty($request->pickup_store_id) and $request->payment_method == 'pickup_from_store') {
			session(['pickup_store_id' => $request->pickup_store_id]);
		}
	...
	{
		// return $request;
		if (isset($request->method_payment) && $request->method_payment == "pickup_from_store") {
			session('shipping_detail')->shipping_price = 0;
		}
		// return $request;
		$payment_status = $this->order->place_order($request);
		$status = Arr::get($payment_status, 'status');
		$orderId = Arr::get($payment_status, 'orders_id');
		// return $payment_status['status'];
		if ($status == 'success') {
			// return $payment_status['orders_id'];
			$orderStatus = Order::where('orders_id', $orderId)->first();
			$orderStatus->payment_status = 1;
			$orderStatus->save();
			$message = "Dear Waqar, We are working on your Order " . $orderId . " It will be delivered within 48 hours.";
	...
	public function getPaymentMethods()
	{
		...
		/**   START Payd **/
		/*************************/
		$payments_setting = $this->order->payments_setting_for_payd();
		if ($payments_setting['secret_key']->environment == '0') {
			$payd_enviroment = 'Test';
		} else {
			$payd_enviroment = 'Live';
		}

		$payd = array(
			'environment' => $payd_enviroment,
		...
		/**   START Postpay Full Installment **/
		/*************************/
		$payments_setting = $this->order->payments_setting_for_postpay_full_installment();
		if ($payments_setting['secret_key']->environment == '0') {
			$postpay_enviroment = 'Test';
		} else {
			$postpay_enviroment = 'Live';
		}

		$postpayFullInstallment = array(
			'environment' => $postpay_enviroment,
			'name' => $payments_setting['secret_key']->name,
			'public_key' => $payments_setting['secret_key']->value,
			'active' => $payments_setting['secret_key']->status,
			'payment_currency' => Session::get('currency_code'),
		...
		/**   START Postpay Installments **/
		/*************************/
		$payments_setting = $this->order->payments_setting_for_postpay_installments();
		if ($payments_setting['secret_key']->environment == '0') {
			$postpay_enviroment = 'Test';
		} else {
			$postpay_enviroment = 'Live';
		...
		$pickup_from_store = array(
			'environment' 		=> 'sandbox',
			'name' 				=> 'Pickup from Store',
			'active' 			=> 1,
			'payment_currency' 	=> Session::get('currency_code'),
			'payment_method' 	=> 'pickup_from_store',
		);
		...
		$result[5] = $hyperpay;
		$result[6] = $payd;
		$result[7] = $pickup_from_store;
		$result[8] = $postpayFullInstallment;
		$result[9] = $postpayInstallments;
	...
	public function payPaydCallback(Request $request)
	{
		if ($request->message == 'Failure') {
			echo 'Something went wrong . . .';
			return redirect()->to('/checkout');
		} else {
			echo 'Do not close the window your order is in process . . .';

			$payments_setting = $this->order->payments_setting_for_payd();
			$payd_secret_key =  $payments_setting['secret_key']->value;

			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => 'https://www.payd.ae/pg/public/api/paymentdetails/' . $request->reference . '/1',
			...
			$content = new Request();
			$content->nonce = $response;
			$this->place_order($content);

			return redirect()->to('/orders');
		}
	}

	public function payPayd(Request $request)
	{
		$commonContent = $this->index->commonContent();
		...
		$payload = array(
			...
			'phone' => $phone_number,
			'email' => $email_id,
			'redirecturl' => URL::to('/pay-payd-callback'),
